...
This repository now contains the first executable UniversalOS foundation: **portable hardware-profile and package-manifest contracts plus a profile-driven compatibility resolver**. It proves that the core can choose packages through declarative data instead of device-name conditionals. It implements the profile/package exact-match portion of the broader [Discovery Base → full-system delivery flow](BOOTSTRAP_DISCOVERY_FLOW.md). Preliminary Bootstrap Capsule selection is separately modeled in [Universal Installer and Bootstrap Capsule contracts](BOOTSTRAP_CONTRACTS.md).
...
3. Replace the Linux reference [local IPC transport](LOCAL_IPC_TRANSPORT.md) with reviewed native service supervision, mandatory-access-control policy, and verified-boot-bound identities.
